...
export function Popup({ add, close }: PopupProps) {
	...
	return (
		<PopupStyled style={style}>
			<PopupWindowStyled>
				<PopupTitleStyled>https://calendar.com</PopupTitleStyled>
				<PopupTextStyled>
					Enter event time:
					<br />
					YYYY-MM-DD HH:mm:ss
				...
				<PopupFormStyled>
					...
					{isError && (
						<PopupStyledError>wrong date format</PopupStyledError>
					)}

					<PoupButtonStyled onClick={handleCancel}>
						Cancel
					</PoupButtonStyled>
					<PoupButtonStyled onClick={handleOk}>Ok</PoupButtonStyled>
				</PopupFormStyled>
			</PopupWindowStyled>
		</PopupStyled>
	);
